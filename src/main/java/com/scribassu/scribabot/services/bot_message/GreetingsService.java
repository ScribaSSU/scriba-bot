        ...
        switch (message) {
            ...
            case CommandText.HELLO:
                if (!registered) {
                    ...
                    return CompletableFuture.completedFuture(new BotMessage(GREETING, innerKeyboardGenerator.mainMenu(botUser), botUser));
                }
            case CommandText.MAIN_MENU:
            case CommandText.SHORT_MAIN_MENU:
                if (!registered) {
            ...
            case CommandText.THANKS:
                return CompletableFuture.completedFuture(new BotMessage(YOU_ARE_WELCOME, innerKeyboardGenerator.mainMenu(botUser), botUser));
        }
        return CompletableFuture.completedFuture(new BotMessage(RETURN_MAIN_MENU, innerKeyboardGenerator.mainMenu(botUser), botUser));
    }
}
