...
public class ExtramuralEventService implements BotMessageService {
    ...
    public BotMessage getBotMessage(String message, BotUser botUser) {
        ...
        if (null == lessons || null == lessons.getExtramuralEvents() || lessons.getExtramuralEvents().isEmpty()) {
            ...
            if(isToday) {
                return BotMessageUtils.getBotMessageForExtramuralEvent(lessons, CommandText.TODAY);
            }
            if(isTomorrow) {
                return BotMessageUtils.getBotMessageForExtramuralEvent(lessons, CommandText.TOMORROW);
            }
            if(isYesterday) {
                return BotMessageUtils.getBotMessageForExtramuralEvent(lessons, CommandText.YESTERDAY);
            }
            return BotMessageUtils.getBotMessageForExtramuralEvent(lessons, "");
        }
    }
}
