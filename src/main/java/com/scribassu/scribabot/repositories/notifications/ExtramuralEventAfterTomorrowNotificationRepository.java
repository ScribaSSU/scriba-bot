...
public interface ExtramuralEventAfterTomorrowNotificationRepository extends JpaRepository<ExtramuralEventAfterTomorrowNotification, Long> {
    ...
    @Query("select s from ExtramuralEventAfterTomorrowNotification s where s.userId = :userId and s.userSource = :userSource")
    ExtramuralEventAfterTomorrowNotification findByUserIdAndUserSource(@Param("userId") String userId,
                                                                       @Param("userSource") BotUserSource userSource);

    @Modifying
    @Transactional
    @Query("update ExtramuralEventAfterTomorrowNotification set isEnabled = true where userId = :userId and userSource = :userSource")
